...
class SlimORMTest extends BaseDbTest {
	...
	public function testAddBookWithContact() {
		...
		$phone->setNumber("608 832 988");

		$phone2 = new \Model\Contact\Entity\Phone();
		$phone2->setNumber("608 832 999");
		...
		$contact->setAddress("Radikovice 6, Lhota Pod Libcany 503 27");
		...
		$this->assertInstanceOf('Model\Library\Entity\Library', $lastBook->library);
		$this->assertEquals($lastBook->author->name, "William Pascal");
		$this->assertEquals($lastBook->author->contact->address, "Radikovice 6, Lhota Pod Libcany 503 27");
		$phones = $lastBook->author->contact->phones;
		$this->assertEquals(current($phones)->number, "608 832 988");
		$this->assertEquals(end($phones)->number, "608 832 999");
	}
